...
import type { CNPJGenerateBatchOptions, CNPJGenerateOptions } from "./cnpj.types.js";
...
export function generateCNPJBatch(options: CNPJGenerateBatchOptions): string[] {
  ...
  if (!options.unique) return Array.from({ length: count }, () => generateCNPJ({ formatted: options.formatted, branch: options.branch }));
  const seen = new Set<string>(); const values: string[] = [];
  while (values.length < count) { const value = generateCNPJ({ formatted: options.formatted, branch: options.branch }); if (!seen.has(value)) { seen.add(value); values.push(value); } }
  return values;
}
