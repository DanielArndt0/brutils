...
import type { CEPGenerateBatchOptions, CEPGenerateOptions } from "./cep.types.js";
...
export function generateCEPBatch(options: CEPGenerateBatchOptions): string[] {
  ...
  return Array.from({ length: count }, () => generateCEP({ formatted: options.formatted, state: options.state }));
}
