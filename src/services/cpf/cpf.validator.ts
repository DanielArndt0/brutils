...
import { calculateCPFCheckDigit, normalizeCPF, resolveCPFStateFromDigits } from "./cpf.shared.js";
import type { CPFValidateOptions, CPFValidationResult } from "./cpf.types.js";
export function validateCPF(value: string, options: CPFValidateOptions = {}): CPFValidationResult {
  const digits = normalizeCPF(value);
  if (digits.length !== 11) return { isValid: false, value: digits };
  if (options.strict && allDigitsEqual(digits)) return { isValid: false, value: digits, formatted: formatCPF(digits) };
  ...
  return { isValid: digits === expectedCPF, value: digits, formatted: formatCPF(digits), state: resolveCPFStateFromDigits(digits) };
}
